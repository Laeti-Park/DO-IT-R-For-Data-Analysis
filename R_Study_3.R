exam <-
  read.csv("Do-it-R-For-Data-Analysis/Data/csv_exam.csv") # read.csv : 기본 내장함수
exam
...
# drv(drivetrain) : 구동 방식(f = 전륜구동, r = 후륜구동, 4ㅍ = 사륜구동)
...
library(dplyr) # 데이터 추출, 필터링, 계산, 열 추가, 요약, 그룹화를 편리하게 할 수 있는 함수 제공
...
mpg$high_test <-
  ifelse(mpg$total >= 20, "pass", "fail") # 고연비 합격 판정을 받은 자동차는 몇 대인가?
mpg$high_test
head(mpg, 10)
table(mpg$high_test)
